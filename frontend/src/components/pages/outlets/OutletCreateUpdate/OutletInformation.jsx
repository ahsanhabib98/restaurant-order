...
class OutletInformation extends Component {
  ...
  existingOrder(outlet) {
    const newData = {
      ...
      same_day: outlet.same_day,
      next_day: outlet.next_day,
      avatar: outlet.avatar,
      phone: outlet.phone,
      email: outlet.email,
      is_publish: outlet.is_publish,
      is_shared: outlet.is_shared,
    };
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.name !== this.state.name) {
      if (nextProps.outlet) {
        this.existingOrder(nextProps.outlet);
      }
    }
  }
  componentDidMount() {
    if (this.props.outlet) {
      this.existingOrder(this.props.outlet);
    }
  }
  ...
  submitHandler = (e) => {
    ...
    if (outletID) {
      this.props.updateOutletAction({ payload, outletID });
    } else {
      this.props.createOutletUpdateAction(payload);
    }
  ...
  render() {
    ...
    const { name, tax_rate, phone, email, is_publish, next_day, same_day } =
      this.state;
    return (
        ...
        <form onSubmit={this.submitHandler}>
          ...
          <div className="form-check">
            <input
              ...
              id="flexCheckDefault"
            />
            <label className="form-checklabel" htmlFor="flexCheckDefault">
              Same day
            </label>
          </div>
          <div className="form-check">
          ...
          <div className="row mt-2">
            ...
            <div className="col-md-4">
              <h6>Tax inclusive</h6>
              <button className="btn btn-primary btn-sm mr-2">Inclusive</button>

              <button className="btn btn-info btn-sm">Exclusive</button>
            </div>
          </div>
          <div className="row mt-4 mb-4">
...
export default connect(null, { createOutletUpdateAction, updateOutletAction })(
  OutletInformation
);
