    ...
    return (
      <div className="mt-5 outletListTable">
        ...
        <table className="table outletTalbe">
            ...
            <tr>
              ...
              <th>
                <a href="#0">
                  Status{" "}
                  <span className="mt-2">
                    <FeatherIcon icon="help-circle" size="13" />
                  </span>
                </a>
              </th>
          ...
          <tbody>
            {searchOutlet &&
              searchOutlet.map((outlet, index) => {
                return (
                  <tr key={index} className="mb-5">
                    <th className="first-column">
                      <img
                        src={outlet.avatar ? outlet.avatar : "logo.svg"}
                    ...
                    <td>
                      {outlet.order_delivery.is_open ? "Enabled" : "Disabled"}
                    </td>
                    <td>
                      {outlet.order_pickup.is_open ? (
                        "Enabled"
                      ) : (
                        <span className="Disabled">
                          Disabled{" "}
                          <span className="ml-3">
                            <Link to={`/outlet/details/${outlet.id}/edit/`}>
