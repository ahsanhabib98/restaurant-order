...
class OTPAdd extends Component {
  ...
  onSubmitHandler = () => {
    ...
    axios
      .get(otpCodeWiseUserURL(phone_number))
      .then((res) => {
        // console.log(res.data);
        if (otp_code === res.data.otp_code) {
          axios
            .put(verifyUserURL(phone_number), { is_verified: true })
            .then((res) => {
              if (this.props.is_owner) {
                axios.put(accountUpdateURL(this.props.user_id), { is_active: false }).then((res) => {
...
const mapStateToProps = (state) => {
  return {
    ...
    user_id: state.login_auth.user.id,
    is_owner: state.login_auth.user.is_owner,
  };
};
