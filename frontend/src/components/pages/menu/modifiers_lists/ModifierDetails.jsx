...
class ModifierDetails extends Component {
  componentDidMount() {
    ...
    this.props.modifierDetailsAction({ id, subdomain });
  }

  render() {
    const { modifier } = this.props;
    const { brandingID, subdomain } = this.props.match.params;
    console.log(brandingID, subdomain);
              ...
              <div className="card p-3 mt-3">
                ...
                <form onSubmit={this.submitHandler}>
                  ...
                  <div className="row mt-2">
                    <div className="col-md-8">
                      <Link
                        to={`/menu/${brandingID}/${subdomain}/modifier-update/${modifier.id}`}
                      >
                        <button className="btn btn-light btn-md">Edit</button>
                      </Link>
                      <Link
                        className="ml-2"
                        to={`/menu/${brandingID}/${subdomain}/modifier`}
                      >
...
export default connect(mapStateToProps, { modifierDetailsAction })(
  ModifierDetails
);
