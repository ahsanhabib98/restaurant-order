...
from .email import invites_email
...
class UserInvitationSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ("email", "branding")
        extra_kwargs = {"branding": {"required": True}}

    def create(self, validated_data):
        if User.objects.filter(email=validated_data.get("email")).exists():
            raise serializers.ValidationError({"email": "User already exists"})
        UserInvitation.objects.filter(email=validated_data.get("email")).delete()
        user = self.context["request"].user
        branding = validated_data.get("branding")
        if (
            branding is None
            or user.company.company_branding.filter(id=branding.id).exists() is False
        ):
            raise serializers.ValidationError({"branding": "Invalid branding data."})

        obj = super().create(validated_data)
        invites_email(user.id, obj.token)
        return obj
